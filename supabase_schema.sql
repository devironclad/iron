...
CREATE TABLE ls_assets (
    -- Identification
    -- id      : autonumber do Postgres, chave primária. Identifica todos os registros internamente.
    --           Usado como identificador de exibição para AUCTION (ex: "ID: 1381").
    -- ref_id  : contador sequencial exclusivo de PROPERTY, atribuído automaticamente via trigger
    --           assign_ref_id (BEFORE INSERT OR UPDATE) quando record_type = 'PROPERTY'.
    --           Exibido no formato PRP-0165 (função formatPropId em src/lib/utils.ts).
    --           Para AUCTION este campo é sempre NULL — nunca deve ser preenchido.
...
-- Atribui ref_id sequencial exclusivamente para registros PROPERTY.
--
-- Regras de negócio:
--   • AUCTION : usa o campo "id" (autonumber) para identificação. ref_id = NULL.
--   • PROPERTY: usa ref_id no formato PRP-XXXX. Atribuído via MAX(ref_id)+1
--               dos registros PROPERTY existentes.
--
-- O trigger dispara em INSERT e UPDATE para cobrir dois cenários:
--   • INSERT direto de uma Property (raro / import).
--   • UPDATE de um Auction existente convertido em Property via fluxo BUY
--     (record_type muda de 'AUCTION' para 'PROPERTY', ref_id chega NULL).
--
-- Não usar SEQUENCE aqui: MAX+1 é auto-recuperável em caso de reset do banco.
-- ==============================================================================

CREATE OR REPLACE FUNCTION assign_ref_id()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.record_type = 'PROPERTY' AND NEW.ref_id IS NULL THEN
...
DROP TRIGGER IF EXISTS trg_assign_ref_id ON ls_assets;
CREATE TRIGGER trg_assign_ref_id
BEFORE INSERT OR UPDATE ON ls_assets
